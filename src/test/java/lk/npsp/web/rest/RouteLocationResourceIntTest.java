...
public class RouteLocationResourceIntTest {
     ...
     *
     * @see lk.npsp.repository.search.RouteLocationSearchRepositoryMockConfiguration
     */
    @Autowired
    private RouteLocationSearchRepository mockRouteLocationSearchRepository;
    ...
    public void setup() {
        ...
        final RouteLocationResource routeLocationResource = new RouteLocationResource(routeLocationRepository, mockRouteLocationSearchRepository);
    ...
    public void createRouteLocation() throws Exception {
        ...
        // Validate the RouteLocation in Elasticsearch
        verify(mockRouteLocationSearchRepository, times(1)).save(testRouteLocation);
    }
    ...
    public void createRouteLocationWithExistingId() throws Exception {
        ...
        // Validate the RouteLocation in Elasticsearch
        verify(mockRouteLocationSearchRepository, times(0)).save(routeLocation);
    }
    ...
    public void deleteRouteLocation() throws Exception {
        ...
        // Validate the RouteLocation in Elasticsearch
        verify(mockRouteLocationSearchRepository, times(1)).deleteById(routeLocation.getId());
    }

    @Test
    @Transactional
    public void searchRouteLocation() throws Exception {
        // Initialize the database
        routeLocationRepository.saveAndFlush(routeLocation);
        when(mockRouteLocationSearchRepository.search(queryStringQuery("id:" + routeLocation.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(routeLocation), PageRequest.of(0, 1), 1));
        // Search the routeLocation
        restRouteLocationMockMvc.perform(get("/api/_search/route-locations?query=id:" + routeLocation.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(routeLocation.getId().intValue())))
            .andExpect(jsonPath("$.[*].sequenceNumber").value(hasItem(DEFAULT_SEQUENCE_NUMBER.intValue())));
